...
const RefundTable = () => {
    ...
    const fetchRefunds = async () => {
        ...
        try {
            ...
            const response = await axios.get('http://localhost:8080/api/payment/refunds', { params });
    ...
    const handleAction = async (id, action) => {
        ...
        try {
            ...
            if (action ==='approve' ){
                await axios.put(`http://localhost:8080/api/payment/payments/update-approve-status/${refund.paymentId}`);
                fetchRefunds();
            }
            if (action ==='reject' ){
                await axios.put(`http://localhost:8080/api/payment/payments/update-reject-status/${refund.paymentId}`);
                fetchRefunds();
            }
