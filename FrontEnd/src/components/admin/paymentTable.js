...
const PaymentTable = () => {
  ...
  const fetchMethodName = async (methodId) => {
    try {
      const response = await axios.get(`http://localhost:8080/payment/payment-methods/${methodId}`);
      return response.data.name || 'N/A';
    } catch (err) {
  ...
  const fetchRefunds = async (paymentId) => {
    try {
      const response = await axios.get(`http://localhost:8080/payment/refunds/${paymentId}`);
  ...
  const handleRefund = async (paymentId,bookingId) => {
    if (window.confirm('Bạn có chắc chắn muốn hoàn tiền cho thanh toán này?')) {
      try {
        ...
        await axios.put(`http://localhost:8080/payment/payments/update-refund-status/${paymentId}`);
        
        // Step 2: Create a new refund record
        await axios.post(`http://localhost:8080/payment/refunds`, {
          paymentId,
