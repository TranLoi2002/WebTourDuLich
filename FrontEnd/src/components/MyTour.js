...
// User-relevant cancellation reasons (values to include)
...
const BookingDetailsModal = ({ isOpen, onClose, booking }) => {
    ...
    const handlePayNow = () => {
        navigate(`/payment/${booking.id}`); // Redirect to payment page
    };

    return (
        <div className="fixed inset-0 bg-gray-600 combine bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-6 max-w-lg w-full">
                <h2 className="text-xl font-bold mb-4">Booking Details</h2>
                <div className="space-y-2">
                    <p><strong>Booking Code:</strong> {booking.bookingCode}</p>
                    <p><strong>Tour:</strong> {booking.tour?.title || 'Unknown Tour'}</p>
                    <p><strong>Booking Date:</strong> {new Date(booking.bookingDate).toLocaleString()}</p>
                    <p><strong>Status:</strong> {booking.bookingStatus}</p>
                    <p><strong>Total Price:</strong> ${booking.totalPrice?.toFixed(2) || '0.00'}</p>
                    <p><strong>Payment Status:</strong> {booking.paymentStatus || 'N/A'}</p>
                    <p><strong>Payment Due:</strong> {booking.paymentDueTime ? new Date(booking.paymentDueTime).toLocaleString() : 'N/A'}</p>
                    <p><strong>Notes:</strong> {booking.notes || 'None'}</p>
                    {booking.bookingStatus === 'CANCELLED' && (
                        <>
                            <p><strong>Reason:</strong> {booking.reason || 'N/A'}</p>
                            <p><strong>Canceled By:</strong> {booking.canceledBy || 'N/A'}</p>
                        </>
                    )}
                    <div>
...
const MyTour = () => {
        ...
        const fetchMyBookings = async () => {
            ...
            try {
                ...
                const bookings = (response.data || []).map(booking => ({
                    ...booking,
                    paymentStatus: booking.bookingStatus === 'PENDING' ? 'Unpaid' :
    ...
    const handleConfirmCancel = useCallback(async (bookingId, reason) => {
